...
@test "moon handler autoload does not define weather providers" (
    functions --erase __tide_report_provider_wttr __tide_report_provider_openmeteo __tide_report_fetch_weather _tide_report_handle_async_moon 2>/dev/null
    source "$repo_root/functions/_tide_report_handle_async_moon.fish"
    not functions -q __tide_report_provider_wttr
    and not functions -q __tide_report_provider_openmeteo
...
@test "moon local handler does not load weather providers" (
    ...
    functions --erase __tide_report_provider_wttr __tide_report_provider_openmeteo __tide_report_fetch_weather __tide_report_provider_moon_local _tide_report_handle_async_moon 2>/dev/null
    set -g tide_report_moon_provider local
    source "$repo_root/functions/_tide_report_handle_async_moon.fish"
    _tide_report_handle_async_moon moon "$HOME/.cache/tide-report/moon.json" 10 20 "NA" red 5
    set -l ok $status
    set -l no_weather (not functions -q __tide_report_provider_wttr; and not functions -q __tide_report_fetch_weather; and echo 0; or echo 1)
    command rm -rf "$tmp"
