...
| **Started** | — |
| **Completed** | — |
| **Depends on** | [phase-02-rule-compliance.md](phase-02-rule-compliance.md) (recommended; wizard extraction overlaps 2.3) |
...
- [ ] **3.1** **Add `functions/tide-report.fish` dispatcher**
...
- [ ] **3.4** **Context-aware wizard prompts in `_tide_report_do_install`**
...
- [ ] **3.8** **Add `tide_report_github_ci_expire_seconds`**
...
- [ ] **3.9** **Implement CI display policy in [`functions/_tide_item_github.fish`](../functions/_tide_item_github.fish)**
...
- [ ] **3.10** **Tests: CI stale → pending, expired → hidden**
...
- [ ] **3.11** **Add `__tide_report_github_auth_ok`**
...
- [ ] **3.12** **Append `!auth` to unavailable text when unauthenticated**
...
- [ ] **3.14** **Tests: fake `gh` auth-status + `!auth` integration**
  - Update [`test/helpers/fake_bin/gh`](../test/helpers/fake_bin/gh).

### GitHub timeout & README

- [ ] **3.15** **Wire GitHub fetch timeout**
...
- [ ] **3.16** **README documentation fixes**
  - `!auth` (implemented).
