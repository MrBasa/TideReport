# tide-report :: Tide Prompt Item
# Displays the next high or low tide from NOAA.

function tide-report_tide --description "Displays US-based tide information in the Tide prompt"
    ...
    if test -z "$tide-report_tide_station_id"
        echo "no station"
        return
    end
    ...
    if not test -f $cache_file
        echo $tide-report_tide_unavailable_text
        _tide-report_fetch $url $cache_file "tide-report_tide_updated"
        return
    end
    ...
    if test $cache_age -gt $tide-report_tide_expire_seconds
        echo $tide-report_tide_unavailable_text
        _tide-report_fetch $url $cache_file "tide-report_tide_updated"
        return
    end
    ...
    if test -z "$next_tide"
        ...
        set -l arrow
        if test "$type" = "H"
            set arrow $tide-report_tide_arrow_rising
        else
            set arrow $tide-report_tide_arrow_falling
        end

        set -l unit_suffix "ft"
        if test "$tide-report_tide_units" = "metric"
            set unit_suffix "m"
        end
    ...
    if test $cache_age -gt $tide-report_tide_refresh_seconds
        _tide-report_fetch $url $cache_file "tide-report_tide_updated"
    end
end
